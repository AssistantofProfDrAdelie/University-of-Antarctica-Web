...
class AppServerTest(unittest.TestCase):
    ...
    def test_ui_contains_complete_encounter_loop(self):
        ...
        self.assertIn('aria-label="查看一袋企鹅微信公众号二维码"', markup)
        self.assertIn("identityLightbox", script)
        ...
        self.assertIn('class="penguin-count"', markup)
        self.assertIn("Professor Adelie has been encountered", markup)
        self.assertIn('id="penguinCountValue"', markup)
        self.assertIn('penguinCountStorageKey="encounter-penguin-count-v1"', script)
        self.assertIn("localStorage.setItem", script)
        self.assertIn("recordEncounter();prepareSave", script)
        self.assertNotIn("hits.sh", markup + script)
        self.assertNotIn("counterapi.com", markup + script)
        self.assertIn("position:fixed", Path("styles.css").read_text())
